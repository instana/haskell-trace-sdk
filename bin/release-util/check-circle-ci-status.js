 ...
 * 1) Inside a Github action, to determine whether the current commit has a green CircleCI pipeline.
 *    This is the default mode when running the script without parameters or with the command
 *    check-for-release.
 *
...
async function checkIfTheCurrentCommitCanBeReleased() {
  ...
  try {
    const pipelines = await executeHttpRequest(
      '/api/v2/project/gh/instana/haskell-trace-sdk/pipeline?branch=main',
      circleToken
    );
    ...
    if (greenMainWorkflows.length >= 1) {
      console.log(
        '\n\nThere is at least one successful main workflow for ' +
...
async function checkIfCommitHasBeenBuiltSuccessfullyOnAnyBranchForAGivenWorkflow() {
  ...
  try {
    ...
    const pipelineIdsForCommit = await fetchWithPagination(
      '/api/v2/project/gh/instana/haskell-trace-sdk/pipeline',
      options,
      0,
      maxPages
    );
      ...
      console.log(
        `\n\nThere is at least one successful workflow "${workflowName}" for commit ${commitHash}.\nThis workflow ` +
          `does not need to be built again for this commit.\nSuccessful builds:\n${greenWorkflows
            .map(
              w =>
                `- https://app.circleci.com/pipelines/github/instana/haskell-trace-sdk/${w.pipeline_number}/workflows/${w.id}`
            )
            .join('\n')}`
      );
      process.exit(0);
